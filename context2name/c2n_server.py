...
class DPLServer(BaseHTTPRequestHandler):
    ...
    def parse_input(self, inp):
        ...
        for line in inp:
            ...
            def translator(x):
                if x.startswith("1ID:-1"):
                    return x.split(":")[2]
                if x.startswith("1ID:0"):
                    return x.split(":")[2]
                if x.startswith("1ID"):
                    return "1ID"
                return x
            ...
            map(
                lambda y: list(
                    map(
                        lambda x: (
                            -x[0],
                            self.omap[2].get(x[1], config.UNKNOWN_TOKEN),
                            x[2],
                        ),
                        y,
                    )
    ...
    def predict(self, inp):
        ...
        encoder_inp = np_utils.to_categorical(
            ctx.reshape([-1]), num_classes=self.imap[0]
        ).reshape([-1, config.N_NEIGHBORS, self.imap[0]])
        ...
        toptens = [
            sorted([(-float(prediction[i][int(j)]), int(j), i) for j in x])
            for i, x in enumerate(toptens)
        ]
...
if __name__ == "__main__":
    ...
    encoder_default = (
        "encoder."
        + str(config.INPUT_VOCAB_SIZE)
        + "_"
        + str(config.HIDDEN_LAYER_SIZE)
        + "."
        + config.MODEL_FILE
    )
    parser.add_argument(
        "-e", type=str, default=encoder_default, dest="encoder", help="Encoder file"
    )

    lstm_default = (
        "lstm_"
        + str(config.HIDDEN_LAYER_SIZE2)
        + "_"
        + str(config.OUTPUT_VOCAB_SIZE)
        + "."
        + config.MODEL_FILE
    )
    parser.add_argument(
        "-m", type=str, default=lstm_default, dest="lstm", help="LSTM Model file"
    )
